...
class BCModuleSplitter {
  ...
  public:
    ...
    static void findAndRecordUsage(llvm::User *U, GlobalVariableInfo &info, llvm::DenseSet<llvm::User *> visited);
    // 分组获取功能
    ...
    getOriginWithOutDegreeFunctions(const llvm::DenseSet<llvm::Function *> &originFuncs);
    llvm::DenseSet<llvm::Function *> getStronglyConnectedComponent(const llvm::DenseSet<llvm::Function *> &originFuncs);

    // BC文件创建
    bool createGlobalVariablesBCFile(const llvm::DenseSet<llvm::GlobalVariable *> &globals, llvm::StringRef filename);
    bool createBCFile(const llvm::DenseSet<llvm::Function *> &group, llvm::StringRef filename, int groupIndex);
  ...
  private:
    // 私有辅助方法
    bool createBCFileWithSignatures(const llvm::DenseSet<llvm::Function *> &group, llvm::StringRef filename,
                                    int groupIndex);
    bool createBCFileWithClone(const llvm::DenseSet<llvm::Function *> &group, llvm::StringRef filename, int groupIndex);
    // Clone模式处理
    void processClonedModuleFunctions(llvm::Module &M, const llvm::DenseSet<llvm::Function *> &targetGroup,
    ...
    getFunctionGroupByRange(const llvm::SmallVector<std::pair<llvm::Function *, int>> &functions, int start, int end);
};
